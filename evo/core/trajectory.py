...
class PosePath3D(object):
    ...
    def __eq__(self, other: object) -> bool:
        ...
        equal &= np.allclose(
            self.orientations_quat_wxyz, other.orientations_quat_wxyz
        ) or np.allclose(
            self.orientations_quat_wxyz, -other.orientations_quat_wxyz
        )
    ...
    def project(self, plane: Plane) -> None:
        ...
        for pose in self._cache.poses_se3:
            ...
            angle_axis = (
                rotation_axis
                * tr.euler_from_matrix(pose[:3, :3], "sxyz")[null_dim]
            )
    ...
    def align(
        ...
        if n == -1:
            r_a, t_a, s = geometry.umeyama_alignment(
                self.positions_xyz.T, traj_ref.positions_xyz.T, with_scale
            )
        else:
            r_a, t_a, s = geometry.umeyama_alignment(
                self.positions_xyz[:n, :].T,
    ...
    def motion_filter(
        ...
        """
        filtered_ids = filters.filter_by_motion(
            self.poses_se3, distance_threshold, angle_threshold, degrees
        )
    ...
    def check(self) -> tuple[bool, dict]:
        ...
        details = {
            "array shapes": (
                "ok" if same_len else "invalid (lists must have same length)"
            ),
            "SE(3) conform": (
                "yes"
                if se3_valid
                else "no (poses are not valid SE(3) matrices)"
            ),
            "quaternions": (
                "ok" if quat_normed else "invalid (must be unit quaternions)"
            ),
        }
...
class PoseTrajectory3D(PosePath3D, object):
    ...
    def speeds(self) -> np.ndarray:
        ...
        return np.array(
            [
                calc_speed(
                    self.positions_xyz[i],
                    self.positions_xyz[i + 1],
                    self.timestamps[i],
                    self.timestamps[i + 1],
                )
    ...
    def sync_with(
        ...
        *,
        ...
        """
        Synchronizes this trajectory with another one in time, see
...
def calc_speed(
    xyz_1: np.ndarray, xyz_2: np.ndarray, t_1: float, t_2: float
) -> float:
    """
